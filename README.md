...
- 685题. Redundant Connection II. <https://leetcode.com/problems/redundant-connection-ii/discuss/108045/cjava-union-find-with-explanation-on>这个解法。具体原理是：  
  ...
  - 然后对于所有的边遍历，对于每一个边的两个节点，`find`父节点，如果相同，而且这时候没有candidate,说明只有环，去掉这条边即可。如果有`candidate`，有谁去掉谁,也就是说去掉candidateA，如果全部搞完没有`parent_x == parent_y`的，说明当前已经是树了，没毛病，去掉`candidateB`即可。
  - 还有一个关键点是，在遍历边的时候如果遇到了`candidate2`要`continue`，因为要求有多个满足条件的边的时候返回后面的。
...
注：如果要求不能重复使用的话不能用`Set`，比如`11:11`就无法通过。要用`counter`.(Google OA1)
- 683题. K Empty Slots. 这道题Java可以用`TreeSet`解决，时间复杂度是`nlog(n)`,用python的情况下,`list.insert()`是O(n),所以复杂度是`O(n^2)`.空间复杂度都是O(n).(Google OA2)
...
- 443题. String Compression.解法是设置一个anchor作为标记，记录已经处理的位置，read 和write分别作为读和写的index,如果不同，那么开始压缩一个字母，后面接上频率， 然后更新anchor.这题难点在于inplace,只需要处理完即可，后面留的尾巴不用管了.
...
## 10.11
- 460题. LFU Cache. 还不会做。
- 239题. Sliding Window Maximum. 跟Min Queue一个思路，`deque`维护一个最大值的队列，维护window大小，一旦对头元素应该`pop`就一定要`pop`，不能用deque长度来判断，因为`deque`长度长期是小于`k`的。
...
- 297题. Serialize and Deserialize Binary Tree. 用dfs来把树撸平，在用dfs把list变成树。
...
- 418题. Sentence Screen Fitting. 这一题预处理字符串，把每个单词之间用空格' '串起来，然后再在后面加一个空格' ', 这样就可以利用一个游标，来计算一共走的步数，除以字符串的长度就能计算出来多少遍。具体操作是:
  ...
  - `if s[start % len] == ' '`这个位置，也就是下一行起始位，是空格' '，说明上一行刚好装了完整的单词，没有出现撕裂，那么因为根据规则要去这个空格，但是我们处理字符串的时候加了一个空格，所以我们要`start + 1`，把这个算进去，为了计算正确的times.
  - `else:`说明这个开头是落在了字母上，
...
- 126题. Word Ladder II,时间复杂度要求很高，backtracking会超时，具体有几种解法:
  + 一种可以先bfs，记录正向边，然后进行bfs.
  + 第二种可以先bfs,记录父节点，也就是反向边，然后dfs从后往前查找。
